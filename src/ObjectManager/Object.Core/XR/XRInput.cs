...
namespace OA.XR
{
    ...
    public sealed class XRInput : MonoBehaviour
    {
        ...
        XRVendor _inputVendor = XRVendor.None;
        bool _running = true;
        ...
        public void Awake()
        {
            ...
            if (vendor == "Oculus") _inputVendor = XRVendor.Oculus;
            else if (vendor == "Openvr") _inputVendor = XRVendor.OpenVR;
            else if (vendor == "Windowsmr") _inputVendor = XRVendor.WindowsMR;
        ...
        public bool GetButton(XRButton button, bool left)
        {
            ...
            else if (button == XRButton.ThumbstickTouch)
            {
                ...
                return GetRawAxis(XRAxis.Trigger, left) > _deadZone;

            else if (button == XRButton.Grip)
                return GetRawAxis(XRAxis.Grip, left) > _deadZone;

            else if (button == XRButton.ThumbstickUp)
                return GetRawAxis(XRAxis.ThumbstickY, left) > _deadZone;

            else if (button == XRButton.ThumbstickDown)
                return GetRawAxis(XRAxis.ThumbstickY, left) < _deadZone * -1.0f;

            else if (button == XRButton.ThumbstickLeft)
                return GetRawAxis(XRAxis.ThumbstickX, left) < _deadZone * -1.0f;

            else if (button == XRButton.ThumbstickRight)
                return GetRawAxis(XRAxis.ThumbstickX, left) > _deadZone;
