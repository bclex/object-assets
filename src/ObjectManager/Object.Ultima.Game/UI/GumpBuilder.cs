...
namespace OA.Ultima.UI
{
    static class GumpBuilder
    {
        public static void BuildGump(Gump gump, string[] gumpPieces, string[] gumpLines)
        {
            ...
            for (var i = 0; i < gumpPieces.Length; i++)
            {
                var gumpParams = Regex.Split(gumpPieces[i], @"\s+");
                switch (gumpParams[0].ToLower())
                {
                    ...
                    case "xmfhtmltok":
                        ...
                        (gump.LastControl as HtmlGumpling).Hue = 0;
                        Utils.Warning($"GUMP: Unhandled {gumpParams[0]}.");
                        break;
                    case "tooltip":
                        ...
                        if (PlayerState.ClientFeatures.TooltipsEnabled)
                        {
                            ...
                            if (gump.LastControl != null)
                                ...
                                Utils.Warning($"GUMP: No control for gump tooltip: {gumpParams[1]}.");
                        }
                        break;
                    case "noresize":
                        ...
                        break;
                    default:
                        Utils.Critical($"GUMP: Unknown piece '{gumpParams[0]}'.");
                        break;
                }
            }
        }
