...
namespace OA.Ultima.Input
{
    class MacroEngine
    {
        List<RunningMacroAction> _runningMacros = new List<RunningMacroAction>();

        public void Run(Action action)
        {
            for (var i = 0; i < _runningMacros.Count; i++)
                if (_runningMacros[i].Action == action)
                {
                    _runningMacros.RemoveAt(i);
                    i--;
                }
            _runningMacros.Add(new RunningMacroAction(action));
        }

        public void Update(double frameMS)
        {
            for (var i = 0; i < _runningMacros.Count; i++)
                if (_runningMacros[i].IsFinished)
                {
                    _runningMacros.RemoveAt(i);
                    i--;
                }
                else _runningMacros[i].Update(frameMS);
        }
