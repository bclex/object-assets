    ...
    public class ESMFile : IDisposable
    {
        ...
        private Record CreateUninitializedRecord(string recordName)
        {
            switch (recordName)
            {
                ...
                case "CREA": return BaeSettings.creaturesEnabled ? new CREARecord() : null;
                case "NPC_": return BaeSettings.npcsEnabled ? new NPC_Record() : null;
                default: Debug.LogWarning("Unsupported ESM record type: " + recordName); return null;
            }
        }
        ...
        private void ReadRecords(string filePath)
        {
            ...
            while (r.BaseStream.Position < r.BaseStream.Length)
            {
                ...
                {
                    // Skip the record.
                    r.BaseStream.Position += recordHeader.dataSize;
                    recordList.Add(null);
                }
            }
