...
namespace OA.Ultima.IO
{
    class FileManager
    {
        ...
        static FileManager()
        {
            ...
            if (Settings.DataDirectory != null && Directory.Exists(Settings.DataDirectory))
            {
                Utils.Debug($"Settings: {Settings.DataDirectory}");
                _fileDirectory = Settings.DataDirectory;
                _isDataPresent = true;
            }
            else
            {
                for (var i = 0; i < _knownRegkeys.Length; i++)
                {
                    ...
                    if (exePath != null && Directory.Exists(exePath))
                    {
                        if (IsClientIsCompatible(exePath))
                        {
                            ...
                            Settings.DataDirectory = exePath;
            ...
            else
            {
                ...
                var patchVersion = string.Join(".", Settings.PatchVersion);
                Utils.Debug($"Client.Exe version: {clientVersion}; Patch version reported to server: {patchVersion}");
                if (!ClientVersion.EqualTo(Settings.PatchVersion, ClientVersion.DefaultVersion))
