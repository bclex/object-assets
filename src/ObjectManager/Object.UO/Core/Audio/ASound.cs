...
namespace OA.Ultima.Core.Audio
{
    abstract class ASound : IDisposable
    {
        ...
        public void Play(bool asEffect, AudioEffects effect = AudioEffects.None, float volume = 1.0f, bool spamCheck = false)
        {
            ...
            switch (effect)
            {
                case AudioEffects.PitchVariation:
                    var pitch = Utility.RandomValue(-5, 5) * .025f;
            ...
            if (buffer != null && buffer.Length > 0)
            {
                ...
                _thisInstance.Play();
                var list = (asEffect) ? _effectInstances : _musicInstances;
                var ms = _thisInstance.GetSampleDuration(buffer.Length).TotalMilliseconds;
        ...
        private void CullExpiredEffects(double now)
        {
            ...
            for (var i = 0; i < _effectInstances.Count; i++)
            ...
            for (var i = 0; i < _musicInstances.Count; i++)
        ...
        {
            var list = asEffect ? _effectInstances : _musicInstances;
            var maxInstances = asEffect ? 32 : 2;
            if (list.Count >= maxInstances) return null;
            else return new DynamicSoundEffectInstance(Frequency, Channels); // shouldn't we be recycling these?
        }
    }
}
