...
namespace OA.Ultima.Core.Network.Compression
{
    /// <summary>
    ...
    public class ZlibCompression
    {
        ...
        const int MinimalCodeLength = 2;
        const int MaximalCodeLength = 11;

        // Fixed overhead, in bits, per compression call
        const int TerminalCodeLength = 4;

        // If our input exceeds this length, we cannot possibly compress it within the buffer
        const int DefiniteOverflow = ((BufferSize * 8) - TerminalCodeLength) / MinimalCodeLength;

        // If our input exceeds this length, we may potentially overflow the buffer
        const int PossibleOverflow = ((BufferSize * 8) - TerminalCodeLength) / MaximalCodeLength;

        static object _syncRoot = new object();

        static byte[] _outputBuffer = new byte[BufferSize];
                    ...
                    fixed (byte* pInputBuffer = input)
                    {
                        ...
                        fixed (byte* pOutputBuffer = _outputBuffer)
                        {
                            ...
                            while (pInput < pInputEnd)
                            {
                                ...
                                {
                                    bitCount -= 8;
                                    if (pOutput < pOutputEnd) *pOutput++ = (byte)(bitValue >> bitCount);
                                    else return null;
                                }
                            }
                            // terminal code
        ...
        public static ZLibError Unpack(byte[] dest, ref int destLength, byte[] source, int sourceLength)
        {
            ...
            catch { return ZLibError.FileError; } // No zlib dll!
        }
    }
