...
namespace OA.Ultima.Core.Network
{
    ...
    {
        static Stack<PacketWriter> _pool = new Stack<PacketWriter>();
        ...
        public void WriteUTF8Null(string value)
        {
            ...
            var length = value.Length;
            _stream.SetLength(_stream.Length + ((length + 1) * 2));
            _stream.Position += Encoding.UTF8.GetBytes(value, 0, length, _stream.GetBuffer(), (int)_stream.Position);
            _stream.Position += 1;
        }
        ...
        public void WriteUTF8Fixed(string value, int size)
        {
            ...
            {
                Encoding.UTF8.GetBytes(value, 0, length, _stream.GetBuffer(), (int)_stream.Position);
                _stream.Position += size;
            }
        }
        ...
        public void Fill(int length)
        {
            ...
            else _stream.Write(new byte[length], 0, length);
        }
