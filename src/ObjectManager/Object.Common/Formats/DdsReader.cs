...
namespace OA.Common.Formats
{
    ...
    public static class DdsReader
    {
        ...
        {
            var alphaFlag = Utils.ContainsBitFlags(pixelFormat.flags, (uint)DDSPixelFormatFlags.AlphaPixels);
            var containsAlpha = alphaFlag || ((pixelFormat.RGBBitCount == 32) && (pixelFormat.ABitMask != 0));
        ...
        private static byte[] DecodeDXT1ToARGB(byte[] compressedData, uint width, uint height, DDSPixelFormat pixelFormat, uint mipmapCount) { return DecodeDXTToARGB(1, compressedData, width, height, pixelFormat, mipmapCount); }
        private static byte[] DecodeDXT3ToARGB(byte[] compressedData, uint width, uint height, DDSPixelFormat pixelFormat, uint mipmapCount) { return DecodeDXTToARGB(3, compressedData, width, height, pixelFormat, mipmapCount); }
        ...
        /// </summary>
        private static void ExtractDDSTextureFormatAndData(DDSHeader header, UnityBinaryReader r, out bool hasMipmaps, out uint DDSMipmapLevelCount, out TextureFormat textureFormat, out int bytesPerPixel, out byte[] textureData)
        {
